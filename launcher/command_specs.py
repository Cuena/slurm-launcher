...
COMMAND_SPECS: dict[str, CommandSpec] = {
    ...
    "download-logs": CommandSpec(
        ...
        agent_recommendation="Use plain text output; dry-run first when checking rsync source paths.",
        supports_json=False,
        examples=("slurm-launcher download-logs --dry-run",),
    ),
    "download-artifacts": CommandSpec(
        summary="Download configured or explicit artifact paths from the tracked remote workdir.",
        agent_recommendation="Use plain text output; prefer explicit `--path` overrides when an agent only needs one artifact.",
        supports_json=False,
        examples=("slurm-launcher download-artifacts --dry-run",),
    ),
}
